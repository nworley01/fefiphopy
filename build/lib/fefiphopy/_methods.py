...
def dFF_generic(data, signal='Gcamp', reference='Isosbestic', freq=12, sample_rate=1220, filter_order=2 ):
    ...
    data['rm_%s'%reference]  = fp.butter_lowpass_filter(df.Isosbestic, freq, 1220, order=5)[1220:]
    scaled_Isos = fp.scale_Isos(data['rm_%s'%reference], data['filt_%s'%signal])
    dFF = (filt_Gcamp-scaled_Isos)/scaled_Isos
